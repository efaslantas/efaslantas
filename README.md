### > whoami

```yaml
name: Emre Ferit Aslantaş
role: DevOps Lead | Platform & MLOps Engineer | kubectl apply -f life.yaml
location: Istanbul, Turkey 🇹🇷
languages: Turkish
experience: 10+ years

domains:
  devops_sre:
    - CI/CD Pipeline Design & GitOps (Jenkins, ArgoCD)
    - Kubernetes & Microservices Architecture
    - Site Reliability Engineering & Incident Management
    - Zero-downtime & Canary Deployments

  platform_infra:
    - On-Prem / Cloud Hybrid Infrastructure (K8s, Proxmox, VMware)
    - Infrastructure as Code (Terraform, Ansible, Helm)
    - Observability Stack (Prometheus, Grafana, ELK)
    - High Availability & Disaster Recovery

  ai_platforms:
    - AI Agent Architectures & Multi-Agent Systems
    - MCP (Model Context Protocol) Server Design
    - LLM Serving & GPU Scheduling
    - MLOps & AI/ML Pipeline Engineering

certifications:
  - GitLab Certified Associate
  - IBM Container & Kubernetes Essentials
  - Docker Essentials - IBM
  - MCSE | MCSA | CCNA
  - FortiGate Security
  - OpenShift Container Platform

motto: "Build systems that run while you sleep."
```

---

### > cat /etc/tech-stack

#### Container & Orchestration
![Kubernetes](https://img.shields.io/badge/Kubernetes-326CE5?style=flat&logo=kubernetes&logoColor=white)
![Docker](https://img.shields.io/badge/Docker-2496ED?style=flat&logo=docker&logoColor=white)
![OpenShift](https://img.shields.io/badge/OpenShift-EE0000?style=flat&logo=redhatopenshift&logoColor=white)
![Rancher](https://img.shields.io/badge/Rancher-0075A8?style=flat&logo=rancher&logoColor=white)
![Helm](https://img.shields.io/badge/Helm-0F1689?style=flat&logo=helm&logoColor=white)
![Istio](https://img.shields.io/badge/Istio-466BB0?style=flat&logo=istio&logoColor=white)

#### CI/CD & GitOps
![Jenkins](https://img.shields.io/badge/Jenkins-D24939?style=flat&logo=jenkins&logoColor=white)
![GitLab CI](https://img.shields.io/badge/GitLab_CI-FC6D26?style=flat&logo=gitlab&logoColor=white)
![Azure DevOps](https://img.shields.io/badge/Azure_DevOps-0078D7?style=flat&logo=azuredevops&logoColor=white)
![ArgoCD](https://img.shields.io/badge/ArgoCD-EF7B4D?style=flat&logo=argo&logoColor=white)
![Ansible](https://img.shields.io/badge/Ansible-EE0000?style=flat&logo=ansible&logoColor=white)
![Terraform](https://img.shields.io/badge/Terraform-844FBA?style=flat&logo=terraform&logoColor=white)
![SonarQube](https://img.shields.io/badge/SonarQube-4E9BCD?style=flat&logo=sonarqube&logoColor=white)

#### Cloud & Infrastructure
![AWS](https://img.shields.io/badge/AWS-232F3E?style=flat&logo=amazonwebservices&logoColor=white)
![Azure](https://img.shields.io/badge/Azure-0078D4?style=flat&logo=microsoftazure&logoColor=white)
![Proxmox](https://img.shields.io/badge/Proxmox-E57000?style=flat&logo=proxmox&logoColor=white)
![Linux](https://img.shields.io/badge/Linux-FCC624?style=flat&logo=linux&logoColor=black)
![Nginx](https://img.shields.io/badge/Nginx-009639?style=flat&logo=nginx&logoColor=white)
![VMware](https://img.shields.io/badge/VMware-607078?style=flat&logo=vmware&logoColor=white)

#### Observability & Monitoring
![Prometheus](https://img.shields.io/badge/Prometheus-E6522C?style=flat&logo=prometheus&logoColor=white)
![Grafana](https://img.shields.io/badge/Grafana-F46800?style=flat&logo=grafana&logoColor=white)
![Elasticsearch](https://img.shields.io/badge/Elasticsearch-005571?style=flat&logo=elasticsearch&logoColor=white)
![Kibana](https://img.shields.io/badge/Kibana-005571?style=flat&logo=kibana&logoColor=white)

#### AI/ML & Agent Platforms
![Python](https://img.shields.io/badge/Python-3776AB?style=flat&logo=python&logoColor=white)
![Go](https://img.shields.io/badge/Go-00ADD8?style=flat&logo=go&logoColor=white)
![MCP](https://img.shields.io/badge/MCP_Servers-412991?style=flat&logo=anthropic&logoColor=white)
![MLOps](https://img.shields.io/badge/MLOps-FF6F00?style=flat&logo=mlflow&logoColor=white)
![NVIDIA](https://img.shields.io/badge/NVIDIA_GPU-76B900?style=flat&logo=nvidia&logoColor=white)
![LLM](https://img.shields.io/badge/LLM_Serving-412991?style=flat&logo=openai&logoColor=white)

#### Databases
![PostgreSQL](https://img.shields.io/badge/PostgreSQL-4169E1?style=flat&logo=postgresql&logoColor=white)
![MongoDB](https://img.shields.io/badge/MongoDB-47A248?style=flat&logo=mongodb&logoColor=white)
![Redis](https://img.shields.io/badge/Redis-DC382D?style=flat&logo=redis&logoColor=white)

---

### > ps aux | grep focus

```shell
# Currently Building
[ACTIVE]  AI Lab K8s         -> Self-hosted GPU cluster for LLM serving & MLOps pipelines
[ACTIVE]  MCP Hub            -> Model Context Protocol server ecosystem on Proxmox

# Platform & Infrastructure
[ACTIVE]  K8s Platform Engineering    -> Multi-cluster management, GitOps, service mesh
[ACTIVE]  Observability & SRE         -> Prometheus, Grafana, ELK, incident response
[ACTIVE]  Infrastructure as Code      -> Terraform, Ansible, Helm charts, CI/CD pipelines
```

---

### > ls -la ~/projects

| Project | Stack | Status |
|---|---|---|
| AI Lab | K8s · GPU · LLM Serving | 🟢 Active |
| MCP Hub | Python · MCP · Proxmox | 🟢 Active |
| Vibe Coding Generator | Python · LLM | 🟡 WIP |

---

### > git log --oneline --graph

<div align="center">

#### Commit Streak
<img src="https://streak-stats.demolab.com/?user=efaslantas&theme=dark&hide_border=true&background=0D1117&ring=00FF00&fire=00FF00&currStreakLabel=00FF00&sideLabels=00FF00&dates=FFFFFF" alt="GitHub Streak" />

#### Contribution Activity
<img src="https://github-readme-activity-graph.vercel.app/graph?username=efaslantas&theme=github-dark&hide_border=true&bg_color=0D1117&color=00FF00&line=00FF00&point=FFFFFF&area=true&area_color=00FF00" width="100%" alt="Contribution Activity" />

#### Profile Overview
<img src="https://github-profile-summary-cards.vercel.app/api/cards/profile-details?username=efaslantas&theme=github_dark" width="100%" alt="Profile Overview" />

</div>

<div align="center">

<img src="https://github-profile-summary-cards.vercel.app/api/cards/repos-per-language?username=efaslantas&theme=github_dark" width="32%" alt="Top Languages (by repo)" />
<img src="https://github-profile-summary-cards.vercel.app/api/cards/most-commit-language?username=efaslantas&theme=github_dark" width="32%" alt="Top Languages (by commit)" />
<img src="https://github-profile-summary-cards.vercel.app/api/cards/productive-time?username=efaslantas&theme=github_dark&utcOffset=3" width="32%" alt="Coding Hours (UTC+3)" />

</div>

---

### > curl -s contact.json

```json
{
  "email": "emreferitaslantas@gmail.com",
  "linkedin": "linkedin.com/in/efaslantas",
  "github": "github.com/efaslantas",
  "timezone": "UTC+3 (Istanbul)"
}
```

<div align="center">

[![LinkedIn](https://img.shields.io/badge/LinkedIn-0077B5?style=for-the-badge&logo=linkedin&logoColor=white)](https://linkedin.com/in/efaslantas)
[![GitHub](https://img.shields.io/badge/GitHub-181717?style=for-the-badge&logo=github&logoColor=white)](https://github.com/efaslantas)
[![Email](https://img.shields.io/badge/Email-EA4335?style=for-the-badge&logo=gmail&logoColor=white)](mailto:emreferitaslantas@gmail.com)

![Profile Views](https://komarev.com/ghpvc/?username=efaslantas&color=00FF00&style=flat-square&label=Visitors)

</div>
